...
// Fallback defaults if backend is not available
const DEFAULTS = {
  proPrice: 199,
  proOriginalPrice: 399,
  enterprisePrice: 399,
  enterpriseOriginalPrice: 799,
  ...
  useEffect(() => {
    getPublicSettings()
      .then(res => {
        ...
        setPricing({
          proPrice:                parseInt(d.proPrice)             || DEFAULTS.proPrice,
          proOriginalPrice:        parseInt(d.proOriginalPrice)     || DEFAULTS.proOriginalPrice,
          enterprisePrice:         parseInt(d.enterprisePrice)      || DEFAULTS.enterprisePrice,
          enterpriseOriginalPrice: parseInt(d.enterpriseOriginalPrice) || DEFAULTS.enterpriseOriginalPrice,
          annualDiscountPct:       parseInt(d.annualDiscountPct)    ?? DEFAULTS.annualDiscountPct,
        });
      })
      .catch(() => {/* silently use defaults */});
  }, []);

  // Build plans dynamically from fetched pricing
  const PRICING_PLANS = [
    {
    ...
    },
    {
      ...
      originalPrice: pricing.proOriginalPrice ? `₹${pricing.proOriginalPrice}` : undefined,
      period: "/month",
      ...
      notIncludedFeatures: [],
      ctaLabel: `Start Pro – ₹${pricing.proPrice}/month`,
      ctaHref: "/signup",
    ...
    },
    {
      ...
      originalPrice: pricing.enterpriseOriginalPrice ? `₹${pricing.enterpriseOriginalPrice}` : undefined,
      period: "/month",
            ...
            <div style={{ display: "inline-flex", background: "#F1F5F9", borderRadius: "12px", padding: "4px", gap: "4px" }}>
              {[["monthly", "Monthly"], ["annual", "Annual"]].map(([val, label]) => (
                <button key={val} onClick={() => setBilling(val)} style={{
                  ...
                  {label}
                  {val === "annual" && (
                    <span style={{ marginLeft: "8px", background: T, color: "#fff", fontSize: "10px", fontWeight: 800, padding: "2px 8px", borderRadius: "99px" }}>{pricing.annualDiscountPct}% OFF</span>
                  )}
                </button>
              ))}
            </div>
            ...
            <div style={{ 
              ...
              {PRICING_PLANS.map((plan, i) => (
                <PricingCard
                  ...
                  {...plan}
                  billing={billing}
                  discountPct={pricing.annualDiscountPct}
