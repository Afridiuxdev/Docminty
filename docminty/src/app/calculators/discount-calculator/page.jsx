        ...
        <div style={{ maxWidth: "760px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }} className="calc-grid">
            <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "24px" }}>
              ...
              <div className="form-field"><label className="field-label">Original Price (Rs.)</label><input className="doc-input" type="number" value={original} onChange={e => setOriginal(e.target.value)} style={{ height: "48px", fontSize: "18px", fontWeight: 700, color: "#374151", fontFamily: "Space Grotesk, sans-serif" }} /></div>
              <div className="form-field">
                <label className="field-label">Discount</label>
                <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                  {[{ v: "percent", l: "Percentage (%)" }, { v: "amount", l: "Fixed Amount (Rs.)" }].map(opt => (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div style={{ background: T, borderRadius: "12px", padding: "24px", textAlign: "center" }}>
                ...
                <p style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 800, fontSize: "40px", color: "#fff", margin: 0 }}>Rs. {fmt(r.finalPrice)}</p>
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.7)", fontFamily: "Inter, sans-serif", margin: "6px 0 0" }}>You save Rs. {fmt(r.discountAmount)}</p>
              </div>
              <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "20px" }}>
                <p className="form-label">Breakdown</p>
                {[["Original Price", `Rs. ${fmt(r.original)}`, "#111827"], ["Discount Amount", `- Rs. ${fmt(r.discountAmount)}`, "#EF4444"], ["Discount %", `${r.discountPercent.toFixed(2)}%`, "#7C3AED"], ["Final Price", `Rs. ${fmt(r.finalPrice)}`, T], ["Total Savings", `Rs. ${fmt(r.discountAmount)}`, "#D97706"]].map(([l, v, c]) => (
              ...
              <div style={{ background: "#FEF9C3", border: "1px solid #F59E0B", borderRadius: "10px", padding: "14px 16px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: "12px", color: "#92400E", fontFamily: "Inter, sans-serif", margin: 0 }}>Save Rs. {fmt(r.discountAmount)} on this purchase</p>
              </div>
            </div>
          </div>
        </div>
