...
export default function AdminNotificationsPage() {
    ...
    const markRead = (id) => setNotifs(n => n.map(x => x.id === id ? { ...x, isRead: true } : x));
                        ...
                        {loading ? (
                            ...
                            return (
                                <div key={n.id} style={{
                                    ...
                                    background: n.isRead ? "transparent" : "#FAFFFE",
                                    borderLeft: n.isRead ? "3px solid transparent" : `3px solid ${T}`,
                                    transition: "background 150ms", cursor: "pointer",
                                }}
                                    onClick={() => markRead(n.id)}
                                    onMouseEnter={e => e.currentTarget.style.background = "#F8F9FA"}
                                    onMouseLeave={e => e.currentTarget.style.background = n.isRead ? "transparent" : "#FAFFFE"}
                                >
                                    <div style={{
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "8px" }}>
                                            ...
                                            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
                                                <span style={{ fontSize: "11px", color: "#9CA3AF", fontWeight: 500, fontFamily: "Inter, sans-serif", whiteSpace: "nowrap" }}>
                                                    {n.time || (n.createdAt ? new Date(n.createdAt).toLocaleString("en-IN", { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : "")}
                                                </span>
                                                <button
                                                    onClick={e => { e.stopPropagation(); dismiss(n.id); }}
                                                    style={{ background: "none", border: "none", cursor: "pointer", padding: "2px", color: "#CBD5E1", display: "flex", transition: "color 150ms" }}
                                        ...
                                        <p style={{ fontSize: "12px", color: "#4B5563", margin: 0, fontFamily: "Inter, sans-serif", lineHeight: 1.5, fontWeight: n.isRead ? 400 : 500 }}>{n.message}</p>
                                    </div>
                                    {!n.isRead && (
