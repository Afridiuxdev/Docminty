...
export async function initiatePayment(planType, onSuccess, onError) {
  try {
    ...
    const options = {
      ...
      name:        "DocMinty",
      description: planType.includes("ENTERPRISE")
        ? (planType.includes("ANNUAL") ? "Enterprise Annual" : "Enterprise Monthly")
        : (planType.includes("ANNUAL") ? "Business Pro Annual" : "Business Pro Monthly"),
      image:       "/favicon.ico",
      prefill: {
        name:  data.userName,
