        ...
        <div style={{ maxWidth: "760px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }} className="calc-grid">
            <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "24px" }}>
              ...
              <div className="form-field"><label className="field-label">Principal Amount (Rs.)</label><input className="doc-input" type="number" value={principal} onChange={e => setPrincipal(e.target.value)} style={{ height: "44px", fontSize: "16px", fontWeight: 700, color: T, fontFamily: "Space Grotesk, sans-serif" }} /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div style={{ background: T, borderRadius: "12px", padding: "24px", textAlign: "center" }}>
                ...
                <p style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 800, fontSize: "40px", color: "#fff", margin: 0 }}>Rs. {fmt(result)}</p>
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.7)", fontFamily: "Inter, sans-serif", margin: "6px 0 0" }}>Total Amount: Rs. {fmt(total)}</p>
              </div>
              <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "20px" }}>
                <p className="form-label">Comparison</p>
                {[["Principal", `Rs. ${fmt(r.P)}`, "#111827"], ["Simple Interest", `Rs. ${fmt(r.si)}`, "#374151"], ["Compound Interest", `Rs. ${fmt(r.ci)}`, T], [`SI Total`, `Rs. ${fmt(r.si_total)}`, "#374151"], [`CI Total`, `Rs. ${fmt(r.ci_total)}`, T], ["Extra via Compounding", `Rs. ${fmt(r.ci - r.si)}`, "#7C3AED"]].map(([l, v, c]) => (
