...
export default function DashboardPage() {
  ...
  const usagePercent = isPro ? 0 : Math.min(100, Math.round((docs.length / 50) * 100));
  ...
  const STATS = [
    ...
    { label: "Plan",            value: user?.plan || "FREE", icon: <Star size={18} color={isPro ? "#F59E0B" : "#9CA3AF"} />, bg: "#F8F9FA" },
  ];
      ...
      <div style={{ padding: "24px" }}>
        ...
        {!isPro && (
          <div style={{ marginBottom: "24px" }}>
            <UsageBar used={docs.length} total={50} label="Documents saved" />
            {docs.length >= 40 && (
              <div style={{ marginTop: "8px", padding: "10px 14px", background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: "8px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ fontSize: "12px", color: "#92400E", margin: 0, fontFamily: "Inter, sans-serif" }}>You are approaching the free plan limit of 50 documents.</p>
                <Link href="/pricing" style={{ fontSize: "12px", fontWeight: 700, color: "#D97706", textDecoration: "none", fontFamily: "Space Grotesk, sans-serif", whiteSpace: "nowrap" }}>Upgrade</Link>
              </div>
            )}
          </div>
        )}
          ...
          <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "20px" }}>
            ...
            {docs.length === 0 ? (
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                ...
                <p style={{ fontSize: "13px", color: "#9CA3AF", margin: "0 0 16px", fontFamily: "Inter, sans-serif" }}>No documents yet. Create your first one!</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {docs.slice(0, 5).map(doc => (
                  <div key={doc.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#FAFAFA", borderRadius: "8px", border: "1px solid #F3F4F6" }}>
                    <div>
                      <p style={{ fontSize: "13px", fontWeight: 600, color: "#111827", margin: "0 0 2px", fontFamily: "Inter, sans-serif" }}>{doc.title}</p>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <button onClick={() => deleteDoc(doc.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#EF4444", fontSize: "14px", padding: "2px" }}>×</button>
                    </div>
                  </div>
                ))}
                {docs.length > 5 && (
                  <Link href="/dashboard/documents" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "4px", padding: "8px", border: "1px dashed #E5E7EB", borderRadius: "8px", textDecoration: "none", fontSize: "12px", color: "#9CA3AF", fontFamily: "Inter, sans-serif" }}>
                    <ArrowRight size={12} /> View {docs.length - 5} more documents
                  </Link>
                )}
              </div>
            )}
        ...
        {!isPro && (
          <div style={{ marginTop: "20px", background: "linear-gradient(135deg,#0D9488,#0F766E)", borderRadius: "12px", padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <p style={{ fontSize: "13px", color: "#99F6E4", margin: 0, fontFamily: "Inter, sans-serif" }}>Batch processing, cloud storage, premium templates — Rs.199/mo</p>
            </div>
