const fs = require('fs');
const itemRowContent = `function ItemRow({ item, index, onChange, onRemove, showHSN, showDiscount }) {
...
apps.forEach(app => {
    ...
    const headStartIdx = content.indexOf("{/* Column headers */}");
    if(headStartIdx !== -1) {
        const itemRowsIdx = content.indexOf("{/* Item rows */}", headStartIdx);
        if(itemRowsIdx !== -1) {
            content = content.substring(0, headStartIdx) + "\n\n                                " + content.substring(itemRowsIdx);
            console.log(app, "Removed column headers");
        }
    }

    const startIdx = content.indexOf("function ItemRow(");
    if(startIdx !== -1) {
        let depth = 0;
        let inFunc = false;
        let endIdx = -1;
        ...
        if (endIdx !== -1) {
           content = content.substring(0, startIdx) + itemRowContent + content.substring(endIdx);
           console.log(app, "Replaced ItemRow");
        }
    }
