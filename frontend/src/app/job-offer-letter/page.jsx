...
export default function JobOfferLetterPage() {
  ...
  const handleDownload = () => {
    download("job-offer", template, form, `OfferLetter-${form.candidateName || "Candidate"}.pdf`);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      payload.file = await generateBlob("job-offer", template, form, `OfferLetter-${form.candidateName || "Candidate"}.pdf`);
            ...
            {activeTab === "terms" && (
              <div>
                ...
                <div className="form-row" style={{ marginTop: "10px" }}>
                  <div className="form-field" style={{ marginBottom: 0 }}><label className="field-label">Working Hours/Day</label><select className="doc-select" value={form.workingHours} onChange={e => updateField("workingHours", e.target.value)}>{["6", "7", "8", "9", "10"].map(h => <option key={h} value={h}>{h} hours</option>)}</select></div>
                </div>
