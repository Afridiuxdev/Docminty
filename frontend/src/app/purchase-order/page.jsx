...
export function POPreview({ form, template = "Classic", accent = "#0D9488" }) {
    ...
    return (
      <div className="pdf-preview" style={{ display: "flex", padding: 0, overflow: "hidden" }}>
        <div style={{ width: "140px", background: accent, padding: "24px 14px", flexShrink: 0, color: "#fff", display: "flex", flexDirection: "column" }}>
          <p style={{ fontSize: "15px", fontWeight: 800, margin: "0 0 4px", fontFamily: "Space Grotesk, sans-serif" }}>PURCHASE</p>
          <p style={{ fontSize: "10px", opacity: 0.75, margin: "0 0 24px" }}>#{form.poNumber}</p>
          ...
          <p style={{ fontSize: "8px", fontWeight: 700, opacity: 0.6, textTransform: "uppercase", margin: "0 0 3px" }}>Vendor</p>
          <p style={{ fontSize: "10px", fontWeight: 600, margin: "0 0 16px" }}>{form.toName || "Vendor Name"}</p>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid #F3F4F6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ textAlign: "right" }}>
              <p style={{ fontSize: "10px", color: "#9CA3AF", margin: 0 }}>Date: {form.poDate}</p>
              {form.deliveryDate && <p style={{ fontSize: "10px", color: "#9CA3AF" }}>Deliv: {form.deliveryDate}</p>}
            </div>
          </div>
          {sharedBody}
        </div>
...
export default function PurchaseOrderPage() {
  ...
  const handleDownload = () => {
    download("purchase", template, form, `PO-${form.poNumber}.pdf`);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = { 
        ...
        title: "PO #" + form.poNumber, 
        referenceNumber: form.poNumber, 
        ...
        <div className="doc-page-wrap">
            ...
            {activeTab === "to" && (
              <div>
                ...
                <div className="form-row" style={{ marginTop: "4px" }}>
                  ...
                  <div className="form-field" style={{ marginBottom: 0 }}><label className="field-label">Email</label><input className="doc-input" type="email" placeholder="vendor@company.com" value={form.toEmail} onChange={e => updateField("toEmail", e.target.value)} /></div>
                </div>
                <div className="form-field"><label className="field-label">Address</label>
          ...
          <div className="preview-panel">
            ...
            <div style={{ position: "relative" }}>
              ...
              <POPreview form={form} template={template} accent={form.templateColor || "#0D9488"} />
            </div>
          </div>
        </div>
