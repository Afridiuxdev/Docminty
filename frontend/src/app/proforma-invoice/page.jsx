...
export default function ProformaInvoicePage() {
  ...
  const handleSave = async () => {
    ...
    const payload = { docType: "proforma-invoice", title: "Proforma #" + form.proformaNumber, referenceNumber: form.proformaNumber, templateName: template, partyName: form.toName, amount: form.items.reduce((s, i) => s + parseFloat(i.amount || 0), 0).toFixed(2), formData: JSON.stringify(form) };
    try {
      const pendingToast = toast.loading("Saving document...");
      payload.file = await generateBlob("proforma", template, form, `Proforma-${form.proformaNumber}.pdf`);
