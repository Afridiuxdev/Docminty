        ...
        <div style={{ maxWidth: "760px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }} className="calc-grid">
            <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "24px" }}>
              ...
              <div className="form-field"><label className="field-label">Cost Price / Total Cost (Rs.)</label><input className="doc-input" type="number" value={cost} onChange={e => setCost(e.target.value)} style={{ height: "48px", fontSize: "18px", fontWeight: 700, color: "#EF4444", fontFamily: "Space Grotesk, sans-serif" }} /></div>
              <div className="form-field"><label className="field-label">Selling Price / Revenue (Rs.)</label><input className="doc-input" type="number" value={revenue} onChange={e => setRevenue(e.target.value)} style={{ height: "48px", fontSize: "18px", fontWeight: 700, color: T, fontFamily: "Space Grotesk, sans-serif" }} /></div>
              <div style={{ padding: "16px", background: r.profit >= 0 ? "#F0FDFA" : "#FEF2F2", border: `1px solid ${r.profit >= 0 ? T : "#FCA5A5"}`, borderRadius: "8px", marginTop: "8px" }}>
                ...
                <p style={{ fontSize: "12px", color: "#6B7280", fontFamily: "Inter, sans-serif", margin: 0 }}>{r.profit >= 0 ? `You make Rs. ${fmt(r.profit)} profit per unit` : `You lose Rs. ${fmt(Math.abs(r.profit))} per unit`}</p>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div style={{ background: r.profit >= 0 ? T : "#EF4444", borderRadius: "12px", padding: "24px", textAlign: "center" }}>
                ...
                <p style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 800, fontSize: "40px", color: "#fff", margin: 0 }}>{r.profit >= 0 ? "" : "-"}{"Rs. "}{fmt(Math.abs(r.profit))}</p>
              </div>
              <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "20px" }}>
                <p className="form-label">Metrics</p>
                {[["Cost Price", `Rs. ${fmt(r.cost)}`, "#EF4444"], ["Revenue", `Rs. ${fmt(r.revenue)}`, T], ["Gross Profit", `Rs. ${fmt(r.profit)}`, r.profit >= 0 ? T : "#EF4444"], ["Profit Margin", `${fmtP(r.margin)}%`, r.margin >= 0 ? T : "#EF4444"], ["Markup", `${fmtP(r.markup)}%`, "#7C3AED"], ["ROI", `${fmtP(r.roi)}%`, "#D97706"]].map(([l, v, c]) => (
