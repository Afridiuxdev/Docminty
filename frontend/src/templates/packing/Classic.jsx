...
export default function PackingClassicTemplate({ form }) {
  ...
  const styles = StyleSheet.create({
    ...
    signature: { height: 45, maxWidth: 140, objectFit: "contain", alignSelf: "flex-end", marginBottom: 4 },
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.addressSection}>
          <View style={{ flex: 1, marginRight: 12 }}>
            <Text style={styles.addressLabel}>From / Sender</Text>
            <Text style={styles.addressName}>{form.fromName || "Your Business"}</Text>
            {form.fromGSTIN && <Text style={styles.addressDetails}>GSTIN: {form.fromGSTIN}</Text>}
            {form.fromAddress && <Text style={styles.addressDetails}>{form.fromAddress}{form.fromCity ? `, ${form.fromCity}` : ""}</Text>}
            {fromState && <Text style={styles.addressDetails}>{fromState}</Text>}
            {form.fromPhone && <Text style={styles.addressDetails}>Ph: {form.fromPhone}</Text>}
            {form.fromEmail && <Text style={styles.addressDetails}>Em: {form.fromEmail}</Text>}
          </View>
          <View style={{ flex: 1, marginRight: 12 }}>
