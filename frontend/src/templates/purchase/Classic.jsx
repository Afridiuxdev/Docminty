...
export default function PurchaseClassicTemplate({ form }) {
  ...
  const styles = StyleSheet.create({
    ...
    signatureImage: { height: 45, maxWidth: 140, objectFit: "contain", alignSelf: "flex-end", marginBottom: 4 },
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.addressRow}>
          <View style={{ flex: 1, marginRight: 12 }}>
            ...
            <Text style={styles.addressName}>{form.fromName || "Your Company"}</Text>
            <View style={styles.addressDetails}>
              {form.fromGSTIN && <Text>GSTIN: {form.fromGSTIN}</Text>}
              {form.fromAddress && <Text>{form.fromAddress}{form.fromCity ? `, ${form.fromCity}` : ""}</Text>}
