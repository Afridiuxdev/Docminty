...
export default function JobOfferCorporateTemplate({ form }) {
  ...
  const styles = StyleSheet.create({
    ...
    header: { backgroundColor: T, padding: "20 24", textAlign: "center", color: "#ffffff" },
    logo: { height: 36, objectFit: "contain", filter: "brightness(0) invert(1)", marginBottom: 8, alignSelf: "center" },
    title: { fontSize: 20, fontFamily: "Space Grotesk", fontWeight: 800, color: "#ffffff", marginBottom: 4 },
    compName: { fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.9)", marginBottom: 2 },
    compInfo: { fontSize: 10, color: "rgba(255,255,255,0.7)", marginBottom: 0 },
    headerDivider: { borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.2)", margin: "8 0" },
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
            ...
            {(form.companyPhone || form.companyEmail) && (
                <Text style={styles.compInfo}>{[form.companyPhone && `Ph: ${form.companyPhone}`, form.companyEmail && `Em: ${form.companyEmail}`].filter(Boolean).join(" | ")}</Text>
            )}
